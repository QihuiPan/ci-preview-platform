...
- Deferred preview replacement after an older namespace is removed must be requested within the pipeline submission time plus its configured TTL. For long queues/builds, choose a sufficiently large TTL; if that replacement window is missed, intentionally submit a new run with --rerun. Existing previews expire based on their recorded ExpiresAt timestamp.
